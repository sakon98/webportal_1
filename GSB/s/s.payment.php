...
<?php
	...
	$strSQL = " SELECT * FROM (     
						SELECT 
							...
							TO_CHAR(KD.RECEIPT_DATE, 'DD/MM/YYYY','NLS_CALENDAR=''THAI BUDDHA') AS A11,
							KPD.SEQ_NO AS SEQ ,
						    '' AS  RECEIPT_NO					
						FROM 
							KPTEMPRECEIVEDET KPD, KPUCFKEEPITEMTYPE KUK , KPTEMPRECEIVE KD
						WHERE 
							KPD.KEEPITEMTYPE_CODE = KUK.KEEPITEMTYPE_CODE AND KPD.KPSLIP_NO = KD.KPSLIP_NO
					...
					UNION
						SELECT 
							...
							TO_CHAR(KM.RECEIPT_DATE, 'DD/MM/YYYY','NLS_CALENDAR=''THAI BUDDHA') AS A11,
							KMD.SEQ_NO AS SEQ,
							KM.RECEIPT_NO AS  RECEIPT_NO
						FROM 
							KPMASTRECEIVEDET KMD, KPUCFKEEPITEMTYPE KUK  , KPMASTRECEIVE KM
						WHERE 
							KMD.KEEPITEMTYPE_CODE = KUK.KEEPITEMTYPE_CODE AND KMD.KPSLIP_NO = KM.KPSLIP_NO
							AND KMD.MEMBER_NO = '$member_no'
							AND KMD.RECV_PERIOD = '$slip_date' AND KMD.KEEPITEM_STATUS = 1
					) ORDER BY SEQ ";
	$value = array('A0','A1','A2','A3','A4','A5','A6','A7','A8','A9','A10','A11','SEQ','RECEIPT_NO');		
