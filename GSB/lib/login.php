...
<?php
 	$getUsr = strip_tags($_POST["usr"]);
        $getUsr = str_replace("{","", $getUsr); $getUsr = str_replace("}","", $getUsr); $getUsr = str_replace(";","", $getUsr); $getUsr = str_replace("[","", $getUsr); $getUsr = str_replace("]","", $getUsr); $getUsr = str_replace("(","", $getUsr); 
        $getUsr = str_replace(")","", $getUsr);
        
	$getPwd =  strip_tags($_POST["pwd"]);
	$getPwd = str_replace("{","", $getPwd); $getPwd = str_replace("}","", $getPwd); $getPwd = str_replace(";","", $getPwd); $getPwd = str_replace("[","", $getPwd); $getPwd = str_replace("]","", $getPwd); $getPwd = str_replace("(","", $getPwd); 
        $getPwd = str_replace(")","", $getPwd);
		...
		if(get_single_value_sql($chk,"chkmember") != 0 or get_single_value_oci($chk,"CHKMEMBER") != 0){		
			if($connection == 0){ // sql db
				if($confirm2use == 0){ // confim disble
					$strSQL = "select password from mbmembmaster where member_no='$getUsr' and id = (select max(id) from mbmembmaster where member_no='$getUsr')";
					...
					$chk1 = "select count(member_no) as chkmember from mbmembmaster where member_no='$getUsr' and who_approve is not null";
						if(get_single_value_sql($chk1,"chkmember") != 0){	
							$strSQL = "select password from mbmembmaster where member_no='$getUsr' and who_approve is not null and id = (select max(id) from mbmembmaster where member_no='$getUsr')";	
							$password = get_single_value_sql($strSQL,"password");
						}else{
	...
	}else if(get_type($getUsr) == "staff"){	// ปรับ sql injection
		$member_no = $getUsr;
