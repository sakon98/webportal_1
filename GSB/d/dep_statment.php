          ...
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <td align="center"><table width="95%" border="0" cellspacing="3" cellpadding="1">
                ...
                <?php 
					$strSQL = "SELECT 
										...
										(select DDS1.PRNCBAL from DPDEPTSTATEMENT DDS1 where DDS1.seq_no = (select max(DDS2.seq_no) as seq_no from DPDEPTSTATEMENT DDS2 where DDS2.DEPTACCOUNT_NO = '$acc_no' 
										AND  DDS2.OPERATE_DATE BETWEEN TO_DATE('$date1', 'DD/MM/YYYY','NLS_CALENDAR=''THAI BUDDHA') 
										AND TO_DATE('$date2', 'DD/MM/YYYY','NLS_CALENDAR=''THAI BUDDHA')) and DDS1.DEPTACCOUNT_NO = '$acc_no') as PRNCBAL_FULL
									FROM 
					...
					$value = array('DEP_DESC','DEPTACCOUNT_NAME','PRNCBAL','PRNCBAL_FULL');
					...
					for($i=0;$i<$Num_Rows;$i++){
						...
						$prncbal_full = $list_info[$i][$j++];
						$j=0;
					}
				?>
                <tr>
                  <td width="10%" height="25" align="left"><strong>ประเภทบัญชี</strong></td>
                  <td width="20%" align="left">เงินฝาก<?=$dep_desc?></td>
				...
				<?php 
					$strSQL = "SELECT 
									...
									WHERE 
										...
										ORDER BY DDS.SEQ_NO  ";
					$value = array('OPERATE_DATE','ITEM_CODE','ITEM_DESC','DEPTITEM_AMT','PRNCBAL','PRNCBAL_FULL');
					list($Num_Rows,$list_info) = get_value_many_oci($strSQL,$value);
					$j=0;
					for($i=0;$i<$Num_Rows;$i++){
            ...
            <tr>
              <td height="30" align="center"><table width="95%" border="0" cellspacing="3" cellpadding="1">
                <tr>
                  <td height="23" align="center"><strong>(-<?=convertthai($prncbal_full)?>-)</strong></td>
                  <td width="15%" align="right"><strong><?=number_format($prncbal_full,2)?></strong></td>
                  <td width="16%" align="center">&nbsp;</td>
                </tr>
              </table></td>
            </tr>
