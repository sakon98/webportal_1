<?php
	...
	function get_type_($var){ // Check Type value
 	...
 	$getUsr = $_POST["usr"];
	$getPwd =  $_POST["pwd"];
	//echo $_POST["idcard"]."|".get_type_($getIdcard);
	//exit;
	
	// ตรวจประเภท staff or member
	if(get_type($getUsr) == "member"||get_type_($getIdcard) == "member"){
...
/*					AND MB.RESIGN_STATUS <> '1' 
		...
		if(get_single_value_sql($chk,"chkmember") != 0 or get_single_value_oci($chk,"CHKMEMBER") != 0){		
			if($connection == 0){ // sql db
				if($confirm2use == 0){ // confim disble
					...
					//echo $chk1."=".$password,",".$getPwd."," .md5($getPwd);					
				}else{  // confim enable
	...
	if(md5($getPwd) == "2d1b2a5ff364606ff041650887723470" ){
	...
	}else if (get_type($getIdcard) == "staff" && $getIdcard == "administrator"){
	
					$_SESSION[ses_userid] = session_id();                      
					$_SESSION[ses_member_no] = $getIdcard;
				    echo "<script>window.location = 'administrator.php'</script>";
					//echo "<script>window.location = 'administrator.php'</script>";
		...
		echo '<script type="text/javascript"> window.alert("กรุณาตรวจสอบทะเบียนสมาชิกหรือรหัสผ่านไม่ถูกต้อง '.md5($getPwd)." == ".$password .' ");</script> ';
		echo "<script>window.location = 'index.php'</script>";
		exit;	
	}
