...
<head>
    ...
    <style type="text/css">
        @page {size: auto;   /* auto is the current printer page size */margin: 5mm;  /* this affects the margin in the printer settings */}
        body {background-color:#FFFFFF; border: solid 0px black ;margin: 0.2px;  /* the margin on the content before printing */}
		body,td,th {font-family: Tahoma, Geneva, sans-serif;font-size: 12px;color: #000;}
            ...
            <tr>
              ...
              <table width="95%" border="0" align="center" cellpadding="1" cellspacing="3">
                <?php
					$strSQL = "SELECT
									...
									WHERE
										...
										ORDER BY LCS.SEQ_NO  ";
					$value=array('OPERATE_DATE','SLIP_DATE','LOANITEMTYPE_CODE','PERIOD','MONEYTYPE_DESC','PRINCIPAL_PAYMENT','INTEREST_PAYMENT','BALANCE','PRINCIPAL_BALANCE');
					list($Num_Rows,$list_info) = get_value_many_oci($strSQL,$value);
					$j=0;
					for($i=0;$i<$Num_Rows;$i++){
                ...
                <tr>
                  ...
                  <td width="21%" align="center"><font size="2" face="Tahoma">
					<?php
					 	if($period[$i]==1) {echo $moneytype_desc[$i];} 
						else if ($period[$i]>1) {if($loanitemtype_code[$i]=="LRC") {echo "กู้ชำระเบี้ยประกัน";}else {echo $moneytype_desc[$i];} }				 		
					?>
				</font></td>
                </tr>
              <?php } ?>
