...
<?php
	...
	//echo $listslip."<br>";
	//echo $slip_date."<br>";
	//echo $listslip_check."<br>";
	

	if($listslip_check == $slip_date){ // 
		...
		$strSQL = "    
						SELECT 
							...
							'' AS A11,
							KPD.SEQ_NO AS SEQ,
		                                        TO_CHAR(K1.RECEIPT_DATE, 'DD/MM/YYYY','NLS_CALENDAR=''THAI BUDDHA') AS A12			
						FROM 
							KPTEMPRECEIVEDET KPD, KPUCFKEEPITEMTYPE KUK , KPTEMPRECEIVE K1
						WHERE 
							KPD.KEEPITEMTYPE_CODE = KUK.KEEPITEMTYPE_CODE AND KPD.KPSLIP_NO = K1.KPSLIP_NO
							AND KPD.MEMBER_NO = '$member_no'
							AND KPD.RECV_PERIOD = '$slip_date' 
							AND KPD.POSTING_STATUS = 0 ORDER BY KPD.SEQ_NO
					";
		...
		$strSQL = "    
						SELECT 
							...
							KMD.item_unkeptamt AS AA7,
							KMD.principal_keptamt AS AA5,
							KMD.interest_keptamt AS AA6,
							KMD.keepitem_status AS KIS,
							...
							'' AS  A11,
							KMD.SEQ_NO AS SEQ,
		                                        TO_CHAR(K2.RECEIPT_DATE, 'DD/MM/YYYY','NLS_CALENDAR=''THAI BUDDHA') AS A12
						FROM 
							KPMASTRECEIVEDET KMD, KPUCFKEEPITEMTYPE KUK , KPMASTRECEIVE K2
						WHERE 
							KMD.KEEPITEMTYPE_CODE = KUK.KEEPITEMTYPE_CODE AND KMD.KPSLIP_NO = K2.KPSLIP_NO
	...
	/*$strSQL = "SELECT * FROM (     
						...
						WHERE 
							KPD.KEEPITEMTYPE_CODE = KUK.KEEPITEMTYPE_CODE AND KPD.KPSLIP_NO = K1.KPSLIP_NO
							AND KPD.MEMBER_NO = '$member_no'
							AND KPD.RECV_PERIOD = '$slip_date' AND KPD.KEEPITEM_STATUS = 1
					...
					UNION
						...
						WHERE 
							KMD.KEEPITEMTYPE_CODE = KUK.KEEPITEMTYPE_CODE AND KMD.KPSLIP_NO = K2.KPSLIP_NO
							AND KMD.MEMBER_NO = '$member_no' AND KMD.KEEPITEM_STATUS = 1
							AND KMD.RECV_PERIOD = '$slip_date'
					) ORDER BY SEQ ";*/
	
	$value = array('A0','A1','A2','A3','A4','A5','A6','A7','A8','A9','A10','A11','SEQ','A12');		
	...
	for($i=0;$i<$Num_Rows;$i++){ 
		...
		$slipdate1		 		= $slip_show[$i][$j++];
		$seq_no		 			= $slip_show[$i][$j++];
