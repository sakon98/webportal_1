	...
	/*$strSQL = "SELECT * FROM (     
					...
					$strSQL = "
						SELECT 
							...
							to_char(DECODE(KMD.PERIOD,0,null,KMD.PERIOD),'9G999G999') AS A8,
							KMD.DESCRIPTION AS A4,
                            (CASE WHEN KMD.KEEPITEM_STATUS = -99 THEN to_char(DECODE(KMD.PRINCIPAL_KEPTAMT,0,null,KMD.PRINCIPAL_KEPTAMT),'99G999G999G999D00') ELSE 
							to_char(DECODE(KMD.PRINCIPAL_PAYMENT,0,null,KMD.PRINCIPAL_PAYMENT),'99G999G999G999D00') END) AS A5,
							(CASE WHEN KMD.KEEPITEM_STATUS = -99 THEN to_char(DECODE(KMD.INTEREST_PAYMENT,0,null,KMD.INTEREST_PAYMENT),'99G999G999G999D00') ELSE 
							to_char(DECODE(KMD.INTEREST_PAYMENT,0,null,KMD.INTEREST_PAYMENT),'99G999G999G999D00') END) A6,
							(CASE WHEN KMD.KEEPITEM_STATUS = -99 THEN to_char(DECODE(KMD.ITEM_KEPTAMT * KUK.SIGN_FLAG ,0,null,KMD.ITEM_KEPTAMT * KUK.SIGN_FLAG),'99G999G999G999D00') ELSE 
							to_char(DECODE(KMD.ITEM_PAYMENT * KUK.SIGN_FLAG,0,null,KMD.ITEM_PAYMENT * KUK.SIGN_FLAG),'99G999G999G999D00') END) A7,
							to_char(DECODE(KMD.ITEM_BALANCE,0,null,KMD.ITEM_BALANCE),'99G999G999G999D00') AS A9,
							(TO_DATE(KMD.CALINTTO_DATE,'DD/MM/YYYY')-TO_DATE(KMD.CALINTFROM_DATE,'DD/MM/YYYY')) AS A10, 
							TO_CHAR(KMD.POSTING_DATE, 'DD/MM/YYYY','NLS_CALENDAR=''THAI BUDDHA') AS  A11,
							KMD.SEQ_NO AS SEQ,
							TO_CHAR(K.RECEIPT_DATE, 'DD/MM/YYYY','NLS_CALENDAR=''THAI BUDDHA') AS RECEIPT_DATE
						FROM 
							KPMASTRECEIVEDET KMD, KPUCFKEEPITEMTYPE KUK,KPMASTRECEIVE K
						WHERE 
							KMD.KEEPITEMTYPE_CODE = KUK.KEEPITEMTYPE_CODE
							AND KMD.KPSLIP_NO = K.KPSLIP_NO
							AND KMD.MEMBER_NO = '$member_no'
							AND KMD.RECV_PERIOD = '$slip_date' ORDER BY KMD.SEQ_NO
					";
					
	$value = array('A0','A1','A2','A3','A4','A5','A6','A7','A8','A9','A10','A11','SEQ','RECEIPT_DATE');		
