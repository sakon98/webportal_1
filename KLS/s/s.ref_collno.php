...
<?php
   ...
   $strSQL2 = " SELECT LCC.COLLACTIVE_AMT AS COLLACTIVE_AMT_OLD,
                       (NVL( LCM.PRINCIPAL_BALANCE, 0 )+NVL( LCM.WITHDRAWABLE_AMT, 0 ))*( LCC.COLLACTIVE_PERCENT / 100 ) AS COLLACTIVE_AMT
	...
	$value2 = array('COLLACTIVE_AMT_OLD','COLLACTIVE_AMT');
	list($Num_Rows2,$list_info2) = get_value_many_oci($strSQL2,$value2);
	$j=0;
	for($i=0;$i<$Num_Rows2;$i++){
	$collactive_amt_old[$i] = $list_info2[$i][$j++];
