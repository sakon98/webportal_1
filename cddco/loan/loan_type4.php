...
<table width="950" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td bgcolor="#FFFFFF">&nbsp;
      <table width="550" border="0" align="center" cellpadding="3" cellspacing="3">
        <tr>
          <td colspan="3" align="center"><strong><font size="3"><img src="images/logo2.png" width="105" height="89" alt=""/>ประมาณการคำนวณการผ่อนชำระเงินกู้พิเศษเพื่อการเคหะสงเคราะห์</font></strong></td>
        </tr>
        <tr>
  ...
  <tr>
    <td bgcolor="#FFFFFF"><table width="940" border="1" align="center" cellpadding="2" cellspacing="2">
      <tr>
        <td width="50" height="33" align="center"><strong>งวดที่</strong></td>
        <td width="190" align="center"><strong>เงินกู้/เงินกู้คงเหลือ</strong></td>
        <td width="143" align="center"><strong>เดือนที่หักเงิน</strong></td>
        <td width="69" align="center"><strong>ดอกเบี้ย</strong></td>
        <td width="70" align="center"><strong>จำนวนวัน</strong></td>
        <td width="117" align="center"><strong>ดอกเบี้ย</strong></td>
        <td width="110" align="center"><strong>เงินต้น</strong></td>
        <td width="123" align="center"><strong>รวมหัก</strong></td>
      </tr>
      <?php 
		...
		$rate = $rate*1;
		$rate1 = (($rate)/100)/12;
		$rate2 = (($rate+0.50)/100)/12;
		
		$period1 = $setPeriod;
		$period2 = $period-$period1;
		
		$step1= round((1-(1/(pow(1+$rate1,$period1-1))))/$rate1,5);		
		$step2= round(pow((1/(1+$rate1)),$period1)*(1+(1-(1/pow((1+$rate2),$period2)))/$rate2),5);
		
		$pay = $loan_amt/($step1+$step2);
		//echo '->';
		$pay = $pay/10;
		$pay = ceil($pay);
		$pay = $pay*10;
	 ...
	 if(intval($d) > 5){
		...
		?>
		<tr>
			<td height="25" colspan="5" align="right" bgcolor="#FFFF66"><strong>รวมยอดปี <?=substr($before_date_e,-4)+543?></strong></td>
		...
		if($row == $loop){?>
				<tr>
				  ...
				  <tr>
					<td height="28" colspan="5" align="right" bgcolor="#0099FF"><strong>รวมทั้งสิ้นตลอดสัญญา</strong></td>
		...
		for($i=0;$i<$period;$i++){  
		  ...
		  <tr>
			...
			<td height="0" align="center"><?=ConvertDate($before_date_e,'bc','num2')?></td>
			<td align="center"><?=number_format($rate,2)?>%</td>
			<td height="0" align="center"><?=$diffdate?></td>
			<td height="0" align="right"><?=number_format($rate_per_day,2)?></td>
			<td height="0" align="right"><?=number_format($loan_per_month,2)?></td>
			<td height="0" align="right"><?=number_format(round($rate_per_day,2)+$loan_per_month,2)?></td>
		  </tr><?php  
