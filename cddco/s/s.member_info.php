...
if($_POST["agree"] == "agree" ){
	...
	$strSQL1_ID = " SELECT COUNT(CARD_PERSON) AS C_ID FROM MBMEMBMASTER WHERE  CARD_PERSON = '$idchk' ";
	$value1_ID = array('C_ID');
	list($Num_Rows_ID,$list_ID) = get_value_many_oci($strSQL1_ID,$value1_ID);
	...
	if($C_ID > 1){
		...
		$status = delete_value_sql_id("mbmembmaster",$idchk);
...
$strSQL = "SELECT MUP.PRENAME_DESC AS PRENAME,
					...
					TO_CHAR(MB.BIRTH_DATE, 'DD-MM-YYYY') AS BIRTH_DATE,
					MB.CARD_PERSON AS CARD_PERSON,
					MB.EMAIL_ADDRESS AS ADDR_EMAIL,
					MB.MEM_TEL AS ADDR_PHONE,
					MB.MEM_TELMOBILE AS ADDR_MOBILEPHONE,
					TO_CHAR(MB.MEMBER_DATE, 'DD-MM-YYYY') AS MEMBER_DATE,
					MBP.POSITION_DESC AS POSITION_DESC,
				...
				WHERE 
					...
					AND MB.RESIGN_STATUS <> '1'
                      AND MB.POSITION_CODE = MBP.POSITION_CODE(+) 
					";
...
$showpay_wf = count_member($member_date,'m');
	if($showpay_wf < 12){
	 ...
	 $strSQL2 = "SELECT  MD.GAIN_NAME || ' (' || MUG.GAIN_CONCERN || ')' AS GAIN_NAME  , MUG.GAIN_CONCERN
                    ...
                    ORDER BY MD.SEQ_NO ";
$value2 = array('GAIN_NAME','GAIN_CONCERN'); 
list($Num_Rows2,$list_info2) = get_value_many_oci($strSQL2,$value2);

$j=0;
for($i=0;$i<$Num_Rows2;$i++){
	    $gain_name[$i] = $list_info2[$i][$j++];
        $gain_concern[$i] = $list_info2[$i][$j++];
	$j=0;
}
?>
