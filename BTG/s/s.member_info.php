...
if($_POST["agree"] == "agree" ){
	...
	mysql_query($strSQL);
	//echo "select count(member_no) as countmemb from mbmembmaster where member_no ='$member_no';<br/> ";
	//echo "select count(idcard) as countidcard from mbmembmaster where idcard ='$idchk' ;<br/> ";
	...
	$countidcard = get_single_value_sql("select count(idcard) as countidcard from mbmembmaster where idcard ='$idchk' ","countidcard");
	 ...
	 $strSQL = "SELECT MUP.PRENAME_DESC AS PRENAME,
					...
					to_char(MB.BIRTH_DATE,'DD') || ' ' ||  (case when to_char(MB.BIRTH_DATE,'MM') = '01' then 'ม.ค.'
						    ...
						    when to_char(MB.BIRTH_DATE,'MM') = '06' then 'ม.ย.'
					  ...
					  to_char(MB.MEMBER_DATE,'DD') || ' ' ||  (case when to_char(MB.MEMBER_DATE,'MM') = '01' then 'ม.ค.'
						    ...
						    when to_char(MB.MEMBER_DATE,'MM') = '06' then 'ม.ย.'
					  ...
					  to_char(MB.BIRTH_DATE,'DD') || to_char(MB.BIRTH_DATE,'MM') || to_char(to_number(to_char(MB.BIRTH_DATE,'YYYY') +543 )) as BD_PS,
...
$value = array('PRENAME','NAME','SURNAME','BIRTH_DATE','CARD_PERSON','ADDR_EMAIL','ADDR_PHONE','ADDR_MOBILEPHONE','MEMBER_DATE','POSITION_DESC','MEMBGROUP_DESC1','SALARY_AMOUNT','SALARY_ID','WEB_CODE','MEMBTYPE_DESC','MEMBGROUP_CODE','ACCUM_INTEREST','YEAR_B','YEAR_M','BD_PS','AGE_B','AGE_M');
