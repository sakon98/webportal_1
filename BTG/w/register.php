	...
	if($register_status){ // เริ่มการสมัคร
		...
		<script>
		 ...
		 function validate_register(){
			 ...
			 if(ValidateEmail(jQuery("#email1").val()) ){
				 window.alert("กรุณากรอก Email ให้ถูกต้อง");   
				return false; 
			 }else */
			 if(jQuery("#mobile1").val().length<10){
				 window.alert("กรุณากรอกเบอร์โทรศัพท์ให้ถูกต้อง");   
				return false; 
			 }else if(jQuery("#pwd_r").val().length<8){
				 window.alert("กรุณากรอกรหัสผ่านให้ถูกต้อง ความยาวอย่างน้อย 8 ตัว");   
				return false; 
			 }else
			 if(jQuery("#pwd_r1").val()!=jQuery("#pwd_r").val()){
				 window.alert("กรุณากรอกรหัสผ่าน ให้ตรงกัน");  
				 return false;
			 }else {
        ...
        <form action="" method="post" id="formID2" >
			 ...
			 <font face="Tahoma" size="2" color="red"><strong><em>* Email เพื่อรับข่าวสารสหกรณ์ กำหนดรหัสผู้ใช้อย่างน้อย 8 ตัวอักษร แต่ไม่เกิน 13 ตัวอักษร</em></strong></font>
              ...
              <input type="button" name="button" id="button2" value="ตกลงสมัคร" data-iconpos="right" data-theme="b" onclick="return validate_register();" />
			  ...
			  <!--<input name="agree" type="hidden" id="agree_" value="agree">-->
              </form>
        <?php
	}
}
...
<?php 
    $valid=false;
	if($_POST["pwd_r"]==$_POST["pwd_r1"]&&strlen($_POST["pwd_r"])>=8){
	    $valid=true;
	}	

	if($_POST["reg"] == "done"&&$valid){  
