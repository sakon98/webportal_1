			...
			/*	$repwd = md5("1234");
                ...
                if(!$update_status){ */ ?>                
                 ...
                 <?php /*}else { 
                     ...
                     ?>
                 	<br/>ระบบได้เปลี่ยนรหัสสมาชิกให้เป็น <font size="4" color="#FF0000">"Sahakorn.xxx"</font> แล้วกรุณาเข้าด้วยรหัสผ่านดังกล่าว<p>  
                 <?php //} ?>         
                </div>
            </div>
...
if($_POST["actions"] == "ลบ"){
	$status = delete_value_sql($_POST["table"],$_POST["id"]);
                    $strSQL_delete = "DELETE FROM check_password WHERE member_no = '".$_POST["member"]."' ";
	...
	if($status){
		...
		//$os = php_uname('n');
			         $os = "";
		$table = "log_action";
		$condition = "(action_do,action_desc,action_id,user,ipconnect,date_log,connectby,system_os)";
		$value  = "('".$action_page."','Delete','".$_POST["member"]."','".$_SESSION[ses_member_no]."','".$ipconnect."','".$date_log."','".$connectby."','".$os."')";
		$status = insert_value_sql($table,$condition,$value);
		if($status){
