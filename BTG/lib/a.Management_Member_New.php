...
if($_POST["update"]=="ปรับปรุง"){
	...
	$strSQL = " update mbmembmaster set email='".$_POST["email"]."',mobile='".$_POST["mobile"]."' where member_no = '$member_no' ";
	mysql_query($strSQL);
	//echo $strSQL;
}
...
if($_POST["button"] == "ค้นหา"){
	if(get_type($_POST["search"]) == "member"){
		...
		$mobile_ = $list_info[0][5];
		$email_ = $list_info[0][6];
	...
	if($Num_Rows != 0){
		...
		<input type="hidden" name="button" value="ค้นหา"/>
		<input type="hidden" name="search" value="<?=$member_no?>"/>
		<input type="hidden" name="menu" value="Management_Member"/>
		  <tr>
		  ...
		  <tr>
			...
			<td align="left">
			 <?php  if($logstatus == 1){ ?>
			    <input type="text" name="mobile" value="<?=$mobile_?>" />
			 <?php }else{ ?>
				<?=$mobile?>
			 <?php } ?>	
			</td>
		  ...
		  <tr>
			...
			<td align="left">
			 <?php  if($logstatus == 1){ ?>
			    <input type="text" name="email" value="<?=$email_?>" />
			 <?php }else{ ?>
				<?=$email?>
			 <?php } ?>	
			</td>
		  ...
		  <?php  if($logstatus == 1){ ?>
			 ...
			 <td colspan="3" align="left">
		      <input type="submit" name="update" id="update" value="ปรับปรุง" class="button1" onclick="return confirm('ท่านต้องการปรับปรุงข้อมูลสมาชิก ใช่ หรือ ไม่ !!!')">
              </td></tr>
		  <?php } ?> 
		  <tr>
		...
		<?php if($confirm2use == 1){	 ?>
		  ...
		  <tr>
            ...
            <a class="popup-alink-1"><b>ดูประวัติการเข้าใช้ 10 ครั้งล่าสุด</b></a>&nbsp;&nbsp;&nbsp; | &nbsp;&nbsp;&nbsp;
           	 	<?php if($repassword == 1){ ?>
            	<a class="popup-alink-2" onclick="return confirm('ท่านต้องการเปลี่ยนรหัสผ่าน เป็น '<?=$card_person?>' ใช่ หรือ ไม่ !!!')"><b>เปลี่ยนรหัสเป็นค่าเริ่มต้น</b></a>
            	<?php  } ?>
            <?php  } ?>
            ...
            <div class="popup-box" id="popup-abox-2">
                ...
                if(!$update_status){ ?>                
                 ...
                 <?php }else { ?>
                 	<br/>ระบบได้เปลี่ยนรหัสสมาชิกให้เป็น <font size="4" color="#FF0000">"<?=$card_person?>"</font> แล้วกรุณาเข้าด้วยรหัสผ่านดังกล่าว<p>  
                 <?php } ?>         
                </div>
            </div>
...
///*
	...
	$sql="delete from mbmembmaster where member_no = '".$_POST["member"]."' ";
 ...
 //*/	
	...
	$table = $_POST["table"];
	$condition = "where member_no = '".$_POST["member"]."' ";
...
<form id="formID2" name="formID2" method="post" action="">
    <tr>
      <td align="center"><strong><font size="2" face="Tahoma">
	     <input type="checkbox" name="checkidcard" id="checkidcard" value="ดูเฉพาะที่เปลี่ยนรหัสผ่าน"/>ดูเฉพาะที่เปลี่ยนรหัสผ่าน
         <input type="submit" name="showall" id="showall" value="แสดงทะเบียนสมาชิกทั้งหมด" class="button5" />
      </font></strong></td>
    </tr>
</form>
  </table>
<?php
if(isset($_POST["showall"])||isset($_GET["showall"])){
		$strSQL = " SELECT m.* FROM mbmembmaster m order by m.date_reg desc,m.member_no asc";
		$value = array('id','date_reg','ipconnect','confirm_date','who_approve','mobile','email','password','idcard','member_no','memb_fullname','date_log');
		list($Num_Rows,$list_info) = get_value_many_sql($strSQL,$value);
		?>
		<table align="center"><tr bgcolor="#CCCCCC"><td>No</td><td>member_no</td><td>วันที่สมัคร</td><td>รหัสผ่าน</td><td>idcard</td><td>Login ล่าสุด</td><td></td></tr>
		<?php
		 $filter=true;
		 
				if(isset($_POST["checkidcard"])){
				  $filter=false;
				}
			$k=0;	
			for($i=0;$i<$Num_Rows;$i++){
			 $j=0;
				$id = $list_info[$i][$j++];
				$date_reg = $list_info[$i][$j++];
				$ipconnect = $list_info[$i][$j++];
				$confirm_date = $list_info[$i][$j++];
				$who_approve = $list_info[$i][$j++];
				$mobile_ = $list_info[$i][$j++];
				$email_ = $list_info[$i][$j++];
				$pass = $list_info[$i][$j++];
				$idcard= trim($list_info[$i][$j++]);
				$member_no_= $list_info[$i][$j++];
				$memb_fullname= $list_info[$i][$j++];
				$date_log=$list_info[$i][$j++];
				$pass_= ($pass!=md5($idcard==null?"-":$idcard)?"<<เปลี่ยนรหัสแล้ว>>":$idcard);
				if($filter||$pass!=md5($idcard==null?"-":$idcard)){
				?>
				<tr><td><?=(++$k)?></td><td><?=$member_no_?></td><td><?=$date_reg?></td><td><?=$pass_?></td><td><?=$idcard?></td><td><?=$date_log?></td><td></td>
				<?php
				}
			}
		?>
