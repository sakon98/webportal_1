...
if($_POST["agree"] == "agree" ){
	...
	// เช็คว่าบัตรประชาชนนี่มีสถานะลาออกหรือไม่
         ...
         $ch = "SELECT COUNT(CARD_PERSON) AS C_CARD_PERSON FROM MBMEMBMASTER WHERE CARD_PERSON = '$idchk'";
         $value_ch = 'C_CARD_PERSON';
         $c_card_person = get_single_value_oci($ch,$value_ch);

        
        if($c_card_person > 1){

        // delete คนที่มีใน mbmembmaster oracle มากกว่า 2 row เป็นเคสคนที่ลาออกเเล้วมาสมัครใหม่
