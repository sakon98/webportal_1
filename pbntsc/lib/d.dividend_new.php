...
<table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
      <?php if($confirmdividend == 1){ ?>
    <td width="86%" align="left"><strong><font size="4" face="Tahoma">ปันผล - เฉลี่ยคืน ประจำปี  <?=$divyear?></font></strong><br />
        <?php } ?>
        <?php if($confirmdividend == 0){ ?>
    <td width="86%" align="left"><strong><font size="4" face="Tahoma">ปันผล - เฉลี่ยคืน ประจำปี  <?=$avgdiv?></font></strong><br />
        <?php } ?>
    ...
    <?php } ?>
     ...
     <?php if($confirmdividend == 0){ ?>
        ...
        <select name="divyear" id="divyear"  onchange="this.form.submit()" >  
			...
			//echo '<option value="'.$mindiv.'">ปี :  '.$mindiv.'</option>';
		...
		if(($maxdiv-$i) < $maxdiv-1){
			...
			echo '<option value="'.($avgdiv-$i).'">ปี :  '.($avgdiv-$i).'</option>';
		}
                //echo $i; exit();
