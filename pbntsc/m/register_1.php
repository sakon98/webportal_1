			...
			<div style=" height:100%; width:80%" > 
				<div style="width:100%; text-align:left;">
					<h4>*** เงื่อนไขและข้อตกลงในการสมัครสมาชิก***</h4>
						<p style="padding-left: 10px;">1.เป็นสมาชิกของสหกรณ์ </p>
				...
				<div>
					<input type="text" name="member_no" id="member_no" value=""  placeholder="เลขสมาชิกสหกรณ์" autocomplete="off" required >
				</div>			 
				<input type="text" name="idchk" id="idchk" value="" placeholder="เลขที่บัตรประชาชน" autocomplete="off" required> 
                               <!-- <input type="password" name="web_codechk" id="web_codechk" value="" placeholder="รหัสผ่านในใบเสร็จ" autocomplete="off" required> -->
...
<?php
                ...
                require "../s/s.member_info_check.php";
                //echo $card_person;
                //echo $Num_Rows;
                ...
                /*if(trim($web_code) != $web_codechk ){ // เลขรหัสใบเสร็จไม่ผ่าน
		$register_status = false;
		echo '<script type="text/javascript"> window.alert("ไม่สามารถดำเนินการได้ รหัสผ่านบนใบเสร็จไม่ถูกต้อง กรุณาตรวจสอบ") </script> ';
		echo "<script>window.location = 'index.php'</script>";
		exit;
	}*/
                    ...
                    <form action="" method="post" id="formID2">
						...
						<div style=" height:100%; width:80%"> 
								...
								<div class="ui-field-contain">
										<label for="memb_no">เลขสมาชิกสหกรณ์:</label>
									<input type="text"  id="memb_no" name="memb_no"  value="<?= $member_no ?>" size="10" readonly>
								</div>
								
								<div class="ui-field-contain">
									<label for="memb_fullname">ชื่อ - นามสกุล:</label>
									<input type="text" class="validate[required]" id="memb_fullname" name="memb_fullname"  value="<?= $full_name ?>" size="35" maxlength="13" readonly>
								</div>

								
								<div class="ui-field-contain">
									<label for="idcard1">เลขที่บัตรประชาชน:</label>
								...
								<div class="ui-field-contain">
									...
									<input type="password" class="validate[minSize[pwd_r]]" id="pwd_r1" name="pwd_r1" maxlength = "16" required>
								</div>
								<div class="ui-field-contain">
			...
			<?php
				if ($_POST["reg"] == "done") {
                                   ...
                                   //echo $memb_fullname = $_POST["memb_fullname"];
				...
				FROM 
					MBMEMBMASTER MB,MBUCFPRENAME MUP

				WHERE MB.PRENAME_CODE = MUP.PRENAME_CODE AND
                                      MB.MEMBER_NO = '".$_POST["memb_no"]."'";
                                $value = array('PRENAME','NAME','SURNAME');
                                list($Num_Rows,$list_info) = get_value_many_oci($strSQL,$value);
                                //echo $Num_Rows;
                                $j=0;
                    for($i=0;$i<$Num_Rows;$i++){
	
                     $full_name_insert = $list_info[$i][$j++].''.$list_info[$i][$j++].'  '.$list_info[$i][$j++];
                     $j=0;
