...
<?php
	...
	$strSQL = "SELECT 
						...
						NVL(TO_CHAR(DECODE(ETC_AMT,0,null,ETC_AMT),'99G999G999G999D00'),0) AS ETC_BALAMT ,
						TO_CHAR(DECODE((DIV_AMT+AVG_AMT+ETC_AMT),0,null,(DIV_AMT+AVG_AMT+ETC_AMT)),'99G999G999G999D00') AS SUMDIV,
						NVL( ( SELECT SUM(MP.EXPENSE_AMT) FROM YRDIVMETHPAY MP WHERE  MP.MEMBER_NO =  '$member_no' AND MP.DIV_YEAR = '$divyear' AND MP.METHPAYTYPE_CODE = 'INS'  ) , 0 ) AS INS,
						DECODE((DIV_AMT+AVG_AMT+ETC_AMT),0,null,(DIV_AMT+AVG_AMT+ETC_AMT)) AS SUMDIV_FULL
	...
	$value = array('DIV_BALAMT','AVG_BALAMT','ETC_BALAMT','SUMDIV','INS','SUMDIV_FULL');
	...
	$ins = $slip_show[0][4];
	$ins_format = number_format($ins,2);  // เอาชื่อตัวแปร $ins_format ไปใส่ช่องหัก
	$sumdiv = $slip_show[0][5];
	$sum_balance = $sumdiv - $ins;
	$sum_balance = number_format($sum_balance,2); // เอาชื่อตัวแปร $sum_balance ไปใส่ช่องปันผลคงเหลือ
