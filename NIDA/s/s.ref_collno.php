...
LCC.COLLACTIVE_AMT						
	...
	$value = array('LOANCONTRACT_NO','PRENAME_DESC','MEMB_NAME','MEMB_SURNAME','MEMBER_NO','PRINCIPAL_BALANCE','COLLACTIVE_AMT');
   ...
   $strSQL2 = " SELECT LCC.COLLACTIVE_AMT , LCM.PRINCIPAL_BALANCE
                     ...
                     AND MEMB.PRENAME_CODE = PRE.PRENAME_CODE
                     AND LCM.CONTRACT_STATUS = '1'
                     AND LCC.LOANCOLLTYPE_CODE = '01' AND LCC.COLL_STATUS = 1 ORDER BY LCM.LOANCONTRACT_NO";
							
	$value2 = array('COLLACTIVE_AMT','PRINCIPAL_BALANCE');
	...
	for($i=0;$i<$Num_Rows2;$i++){
		$collactive_amt[$i] = $list_info2[$i][$j++];
				...
				$principal_balance[$i] = $list_info2[$i][$j++];
                $principal_balance[$i] = number_format($principal_balance[$i], 2);
		$j=0;
	}	 
