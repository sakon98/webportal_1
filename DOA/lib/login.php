...
<?php
	...
	if(get_type($getUsr) == "member"){
		...
		// เช็คว่าสมัคร webportal หรือยัง
		...
		if($my == 0) { // ไม่มี row ใน mbmembmaster ใน mysql
		
		// get ค่า max id จาก mbmembmaster ใน mysql มา +1 เตรียมยิงลง mbmembmaster 
		...
		// ดึงข้อมูลจาก mbmembmaster oracle มา insert mysql
		...
		$strSQL_register1 = "select 
						...
						while($objResult_register1 = oci_fetch_array($objParse1,OCI_BOTH)){
							 ...
							 // insert ข้อมูลลง mbmembmaster mysql
