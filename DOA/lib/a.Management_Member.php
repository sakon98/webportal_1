...
if($_POST["button"] == "ค้นหา"){
	...
	$idcard = "0";
		...
		$strSQL = " SELECT * FROM mbmembmaster where member_no = '$member_no' or idcard = '$idcard'"; 
		$value = array('id','date_reg','ipconnect','confirm_date','who_approve','member_no');
		...
		<table width="85%" border="0" align="center" cellpadding="1" cellspacing="6">
		  ...
		  <tr>
			...
			<td align="right">มือถือ :</td>
                          ...
                          <?php 
                $strSQL1 = " SELECT mobile FROM mbmembmaster where member_no = '$member_no' ";
		$value1 = array('mobile');
		list($Num_Rows1,$list_info1) = get_value_many_sql($strSQL1,$value1);
		$mobile_web = $list_info1[0][0];

                ?>
                        
			<td align="left"><?=$mobile_web?></td>
		  </tr>
		  <tr>
			...
			<td align="right">อีเมล์ :</td>
                ...
                $strSQL1 = " SELECT email FROM mbmembmaster where member_no = '$member_no' ";
		$value1 = array('email');
		list($Num_Rows1,$list_info1) = get_value_many_sql($strSQL1,$value1);
		$email_web = $list_info1[0][0];

                ?>
                        
			<td align="left"><?=$email_web?></td>
		  </tr>
		  <tr>
