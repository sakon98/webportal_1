...
<?php
	...
	// get_slip
	$strSQL_get_slip = "SELECT RECEIPT_NO AS RECEIPT_NO FROM KPMASTRECEIVE   WHERE  MEMBER_NO = '$member_no' AND RECV_PERIOD = '$slip_date'";
	...
	$strSQL = "select
					...
					decode(kpucfkeepitemtype.keepitemtype_grp,'LON', kptempreceivedet.principal_payment - kpmastreceivedet.adjust_prnamt, kptempreceivedet.principal_payment) as A5,
					decode(kpucfkeepitemtype.keepitemtype_grp,'LON', kptempreceivedet.interest_payment - kpmastreceivedet.adjust_intamt,kptempreceivedet.interest_payment) as A6,
					kptempreceivedet.shrlontype_code,   
  
					decode(kpucfkeepitemtype.keepitemtype_grp,'SHR', kptempreceivedet.item_balance - kpmastreceivedet.adjust_prnamt,kptempreceivedet.item_balance + kpmastreceivedet.adjust_prnamt) as A9,   
					...
					( kptempreceivedet.principal_payment - kpmastreceivedet.adjust_prnamt) + (kptempreceivedet.interest_payment - kpmastreceivedet.adjust_intamt),
					decode(kpucfkeepitemtype.keepitemtype_grp,'SHR', ( kptempreceivedet.item_payment - kpmastreceivedet.adjust_prnamt),decode(kpucfkeepitemtype.keepitemtype_grp,'dep',( kptempreceivedet.item_payment - kpmastreceivedet.adjust_prnamt) ,kptempreceivedet.item_payment))) as A7,
	...
	if($Num_Rows == 0){
		$strSQL = "select  
					kpmastreceive.receipt_no as A0,
					kpucfkeepitemtype.keepitemtype_grp as A1,
					kpucfkeepitemtype.keepitemtype_desc as A2,
					kpmastreceivedet.loancontract_no as A3,
					kpmastreceivedet.member_no ,
					kpmastreceivedet.recv_period , 
					kpmastreceivedet.keepitemtype_code ,
					kpmastreceivedet.description as A4,
					kpmastreceivedet.period as A8,
					kpmastreceivedet.principal_payment as A5,
					kpmastreceivedet.interest_payment as A6,
					kpmastreceivedet.item_payment as A7,
					kpmastreceive.keeping_status ,
					kpmastreceivedet.keepitem_status ,
					kpmastreceivedet.ref_membno ,
					kpmastreceive.member_no ,
					kpucfkeepitemtype.sort_in_receive ,
					kpmastreceivedet.item_balance as A9,
					kpmastreceivedet.shrlontype_code ,
					kpucfkeepitemtype.sign_flag ,
					TO_CHAR(kpmastreceive.receipt_date, 'DD/MM/YYYY','NLS_CALENDAR=''THAI BUDDHA') as a10			
				from 
					kpmastreceive ,
					kpmastreceivedet ,
					kpucfkeepitemtype
				where  
					( KPMASTRECEIVE.coop_id = KPMASTRECEIVEDET.coop_id ) and
					...
					( kpmastreceive.member_no = '$member_no' ) and
					( kpmastreceive.recv_period = '$slip_date') and
