...
$strSQL = "SELECT
					...
					(SELECT TO_CHAR(OPERATE_DATE, 'DD/MM/YY','NLS_CALENDAR=''THAI BUDDHA') FROM DPDEPTSTATEMENT WHERE DEPTACCOUNT_NO=DM.DEPTACCOUNT_NO AND SEQ_NO=DM.LASTSTMSEQ_NO) AS OPERATE_DATE 
				FROM 
					DPDEPTMASTER DM 
			WHERE 
					(DM.MEMBER_NO='$member_no' OR DM.DEPTACCOUNT_NO IN (SELECT DEPTACCOUNT_NO FROM DPDEPTMASTER WHERE  NVL(INSTR(MEMBER_REF,'00011460'),0) <> 0))
