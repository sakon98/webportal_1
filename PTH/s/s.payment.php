...
<?php
	...
	//echo $slip_date;
	 ...
	 $strSQL = "SELECT * FROM (     
						SELECT 
							...
							KPD.SEQ_NO AS SEQ,
                            SUBSTR(KPD.KEEPITEMTYPE_CODE,1,1) as KEEPITEMTYPE_CODE					
						FROM 
					...
					UNION
						SELECT 
							...
							KMD.SEQ_NO AS SEQ,
                            SUBSTR(KMD.KEEPITEMTYPE_CODE,1,1) as KEEPITEMTYPE_CODE
						FROM 
	...
	$value = array('A0','A1','A2','A3','A4','A5','A6','A7','A8','A9','A10','A11','SEQ_NO','KEEPITEMTYPE_CODE');		
	...
	for($i=0;$i<$Num_Rows;$i++){ 
		...
		$slipdate[$i]		 		= $slip_show[$i][$j++];
		$seq_no[$i]		 		= $slip_show[$i][$j++];
		$keepitemtype_code[$i]		 		= $slip_show[$i][$j++];
		$j=0;
