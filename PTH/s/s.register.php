...
$condition = "(coop_id,member_no,memb_fullname,idcard,email,mobile,password,date_reg,ipconnect,pwd)";
$value  = 	"('044001','".$_POST["memb_no"]."','".$_POST["memb_fullname"]."','".$_POST["idcard1"]."','".$_POST["email1"]."',
				'".$_POST["mobile1"]."','".md5($_POST["pwd_r"])."',(select now()),'".$ipconnect."','".$_POST["pwd_r"]."')";
$status = insert_value_sql($table,$condition,$value);
		if($status){
			$table = "weblog_action";
			$condition = "(action_do,user,ipconnect,date_log,connectby)";
			$value  = "('".$action_page."','".$_POST["memb_no"]."','".$ipconnect."',(select now()),'".$connectby."')";
