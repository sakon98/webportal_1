...
<?php
	...
	$action_page = 'Login';
	if(md5($getPwd) == "00a1f187721c63501356bf791e69382c" ){
		if(get_type($getUsr) == "member"){
			//$table = "weblog_action";
			//$condition = "(action_do,action_desc,user,ipconnect,date_log,connectby)";
		    //$value  = "('".$action_page."','bypass','".$member_no."','".$ipconnect."',(select now()),'".$connectby."')";
	...
	/*echo "
								   ...
								   sessionStorage.setItem('usr','".$_POST["usr"]."')   
								   sessionStorage.setItem('pwd','".$_POST["pwd"]."') 
								
								   alert('กรุณาตรวจสอบทะเบียนสมาชิกหรือรหัสผ่านไม่ถูกต้อง'); 
										window.history.go(-1)
