...
<?php
//echo $member_no;
$strMySQL = "SELECT id,concat(concat(concat(concat(substr(date_format(date_log,'%d/%m/%Y'),1,2),'/'),substr(date_format(date_log,'%d/%m/%Y'),4,2)),'/'),substr(date_format(date_log,'%d/%m/%Y'),7,4) +543) as date_log,action_do,ipconnect,connectby FROM weblog_action where user = '$member_no' order by id desc LIMIT 0 , 10";
