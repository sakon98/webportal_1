...
if($_POST["agree"] == "agree" ){
	...
	$strSQL = "SELECT MUP.PRENAME_DESC AS PRENAME,
					...
					MUP.POSITION_DESC AS POSITION_DESC,
					...
					TO_CHAR(DECODE(S.SHARESTK_AMT*10,0,null,S.SHARESTK_AMT*10),'99G999G999G999D00') AS SHARESTK_AMT,
                      TO_CHAR(SYSDATE, 'DD/MM/YYYY HH24:MI:SS', 'NLS_CALENDAR=''THAI BUDDHA''NLS_DATE_LANGUAGE=THAI') AS S_DATE,
                      TO_CHAR(DECODE(S.SHAREBEGIN_AMT*10,0,null,S.SHAREBEGIN_AMT*10),'99G999G999G999D00') AS SHAREBEGIN_AMT ,
                      MUL.LEVEL_DESC
				FROM 
					MBMEMBMASTER MB,
					MBUCFPRENAME MUP,
					MBUCFMEMBGROUP MBG1,
					MBUCFMEMBTYPE MBT,
					SHSHAREMASTER S , MBUCFLEVEL MUL , MBUCFPOSITION MUP
				WHERE 
					...
					AND MB.MEMBER_NO = S.MEMBER_NO(+)
                      ...
                      AND MB.POSITION_CODE = MUP.POSITION_CODE(+)
					AND MB.MEMBER_NO = '$member_no' 
          			AND MB.RESIGN_STATUS = '0'";
/*					AND MB.RESIGN_STATUS <> '1' 
					AND MB.DEAD_STATUS <> 1 */
$value = array('PRENAME','NAME','SURNAME','BIRTH_DATE','CARD_PERSON','ADDR_EMAIL','ADDR_PHONE','ADDR_MOBILEPHONE','MEMBER_DATE','POSITION_DESC','MEMBGROUP_DESC1','SALARY_AMOUNT','SALARY_ID','WEB_CODE','MEMBTYPE_DESC','MEMBGROUP_CODE','ACCUM_INTEREST','SHARESTK_AMT','S_DATE','SHAREBEGIN_AMT','LEVEL_DESC');
...
for($i=0;$i<$Num_Rows;$i++){
	...
	$s_date = $list_info[$i][$j++];
	$sharebegin_amt = $list_info[$i][$j++];
