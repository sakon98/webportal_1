...
if($_POST["ref"] == "addnew"){	
	...
	if($status){
		...
		$value  = "('".$action_page."','Add','".$action_id."','".$member_no."','".$ipconnect."',now(),'".$connectby."')";
		$status = insert_value_sql($table,$condition,$value);
		if($status){
...
if($_POST["actions"] == "Delete"){
	...
	if($status){
		...
		$table = "weblog_action";
		$condition = "(action_do,action_desc,action_id,user,ipconnect,date_log,connectby)";
		$value  = "('".$action_page."','Delete','".$action_id."','".$member_no."','".$ipconnect."',now(),'".$connectby."')";
		$status = insert_value_sql($table,$condition,$value);
		if($status){
	...
	if(update_value_sql($table,$condition,$value)){
		...
		$table = "weblog_action";
		$condition = "(action_do,action_desc,action_id,user,ipconnect,date_log,connectby)";
		$value  = "('".$action_page."','update','".$_POST["id"]."','".$member_no."','".$ipconnect."',now(),'".$connectby."')";
		$status = insert_value_sql($table,$condition,$value);
		if($status){
