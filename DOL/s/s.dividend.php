...
<?php
	$strSQL = "SELECT MAX(YM.DIV_YEAR) AS MAXDIV ,MIN(YM.DIV_YEAR) AS MINDIV FROM YRDIVMASTER YM WHERE YM.MEMBER_NO = '$member_no'";
	...
	$strSQL = "SELECT 
						DIV_AMT AS DIV_BALAMT ,
						AVG_AMT AS AVG_BALAMT ,
						TO_CHAR(DECODE((DIV_AMT+AVG_AMT),0,null,(DIV_AMT+AVG_AMT)),'99G999G999G999D00') AS SUMDIV,
(SELECT SUM(EXPENSE_AMT) AS DIV_AMT FROM YRDIVMETHPAY WHERE MEMBER_NO = '$member_no' AND DIV_YEAR = '$divyear' AND METHPAYTYPE_CODE IN ('FSC','LED','LON','FCT')) AS ETC_BALAMT
					FROM 
						YRDIVMASTER 
					WHERE 
						MEMBER_NO = '$member_no'
						AND DIV_YEAR = '$divyear' ";
	$value = array('DIV_BALAMT','AVG_BALAMT','SUMDIV','ETC_BALAMT');
	list($Num_div,$slip_show) = get_value_many_oci($strSQL,$value);
	$div_balamt = $slip_show[0][0]; 
	$avg_balamt = $slip_show[0][1];
	$etc_balamt = $slip_show[0][3];
	$sumdiv = $slip_show[0][2];
        
        $sumdiv2 = $div_balamt + $avg_balamt - $etc_balamt;
	...
	$strSQL1 = "SELECT
						...
						WHERE 
							...
							AND DIV.DIV_YEAR= '$divyear' AND METHPAYTYPE_CODE IN ('CSH','CBT','NXT')";
