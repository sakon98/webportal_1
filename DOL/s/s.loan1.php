...
$strSQL_C1 = "SELECT MAX(RECV_PERIOD) AS C1 FROM KPMASTRECEIVEDET WHERE MEMBER_NO = '$member_no'";
	$value_c1 = ('C1');
	 $showslip = get_single_value_oci($strSQL_C1,$value_c1);
 ...
 $strSQL = "SELECT 
					...
					(SELECT SUM(PRINCIPAL_PAYMENT) FROM KPMASTRECEIVEDET WHERE LOANCONTRACT_NO=LNM.LOANCONTRACT_NO AND RECV_PERIOD='$showslip') AS PRINCIPAL_PAYMENT, 
					(SELECT SUM(INTEREST_PAYMENT) FROM KPMASTRECEIVEDET WHERE LOANCONTRACT_NO=LNM.LOANCONTRACT_NO AND RECV_PERIOD='$showslip') AS INTEREST_PAYMENT,
					((SELECT SUM(PRINCIPAL_PAYMENT) FROM KPMASTRECEIVEDET WHERE LOANCONTRACT_NO=LNM.LOANCONTRACT_NO AND RECV_PERIOD='$showslip')+
					(SELECT SUM(INTEREST_PAYMENT) FROM KPMASTRECEIVEDET WHERE LOANCONTRACT_NO=LNM.LOANCONTRACT_NO AND RECV_PERIOD='$showslip')) AS TOTALPAY,
