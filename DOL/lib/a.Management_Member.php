...
if($_POST["button"] == "ค้นหา"){
	if(get_type($_POST["search"]) == "member"){
		...
		$strSQL = " SELECT id,concat(concat(concat(concat(substr(date_format(date_reg,'%d/%m/%Y'),1,2),'/'),substr(date_format(date_reg,'%d/%m/%Y'),4,2)),'/'),substr(date_format(date_reg,'%d/%m/%Y'),7,4) +543) as date_reg,ipconnect,confirm_date,who_approve FROM webmbmembmaster where member_no = $member_no ";
		...
		<?php if($confirm2use == 1){	 ?>
		  ...
		  <tr>
		    ...
		    <td colspan="3" align="left"><form id="form1" name="form1" method="post" action="">
              ...
              <input name="table" type="hidden" value="webmbmembmaster" />
              <input name="member" type="hidden" value="<?=$member_no?>" />
		    </form>   
            <?php  if($logstatus == 1){ ?>
            <a class="popup-alink-1"><b>ดูประวัติการเข้าใช้ 10 ครั้งล่าสุด</b></a>&nbsp;&nbsp;&nbsp; | &nbsp;&nbsp;&nbsp;
           	 	<?php if($repassword == 1){ ?>
            	<button onclick="OpenPopupCenter('repassword.php', 'เปลี่ยนรหัสเป็นค่าเริ่มต้น', 400, 200);">เปลี่ยนรหัสผ่านเป็นค่าเริ่มต้น</button>
            	<?php  } ?>
            <?php  } ?>
...
if($_POST["actions"] == "ลบ"){
	...
	if($status){
		...
		$table = "weblog_action";
		$condition = "(action_do,action_desc,action_id,user,ipconnect,date_log,connectby)";
		$value  = "('".$action_page."','Delete','".$_POST["member"]."','".$_SESSION[ses_member_no]."','".$ipconnect."',now(),'".$connectby."')";
		$status = insert_value_sql($table,$condition,$value);
		if($status){
	...
	if(update_value_sql($table,$condition,$value)){
		...
		$table = "weblog_action";
		$condition = "(action_do,action_desc,action_id,user,ipconnect,date_log,connectby)";
		$value  = "('Approve','".$action_page."','".$_POST["member"]."','".$_SESSION[ses_member_no]."','".$ipconnect."',now(),'".$connectby."')";
		$status = insert_value_sql($table,$condition,$value);
		if($status){
